...
function AssignmentsSection({ courseId }) {
  ...
  return (
    <div>
      <Link to={`/assignments/${courseId}`} className="btn btn-primary-custom mb-3">
        <i className="bi bi-arrow-right"></i> Xem tất cả bài tập
      </Link>
      <div className="row">
        {assignments.slice(0, 3).map((assignment) => (
          <div key={assignment.id} className="col-md-4 mb-3">
            <div className="card border">
              <div className="card-body">
                <h6>{assignment.tieu_de}</h6>
                <small className="text-muted d-block">
                  Hạn: {assignment.han_nop ? new Date(assignment.han_nop).toLocaleDateString('vi-VN') : 'Không có hạn'}
                </small>
                {assignment.is_required && (
                  <span className="badge bg-danger mt-2">Bắt buộc</span>
                )}
              </div>
            </div>
          </div>
